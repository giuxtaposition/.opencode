...
**Context order matters:** `shouldUpdateComponent` means _you_ update the component, while `shouldComponentUpdate` means _component_ updates itself.
...
| -------- | ------------------------------------- | -------- |
| `remove` | Removes something _from_ a collection | `add`    |
...
const errors = [new Error("Network failed"), new Error("Timeout")];
